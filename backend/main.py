...
def create_agent(memory: ConversationBufferMemory) -> AgentExecutor:
    ...
    "- After successful payment, the system will email a receipt and update Google Sheets inventory. "
    ...
    "- Then start payment by calling `trigger_payment_tool` with ONLY the current cart items.\n"
    "- After that, tell the user that the payment form or payment link is ready. "
    "Do not show raw tool payloads or Stripe IDs.\n\n"

    "4) Payment Verification:\n"
    "- When the user says they paid, call `stripe_checkout_status_tool`.\n"
    "- If payment is NOT 'paid', inform the user that payment is not completed yet and stop.\n"
    "- If payment IS 'paid', acknowledge clearly and proceed to finalize the order.\n\n"

    "5) Finalizing a Paid Order:\n"
    "- Ask the user for their email for the receipt.\n"
    "- Then call `place_order` ONCE with: "
    "{{ session_id, customer_email, items:[{{name, qty}}] }}. "
    "These items must ONLY be the items they just paid for.\n"
    "- The backend will:\n"
    "  • Decrement stock and increment order counts in Google Sheets\n"
    "  • Send the receipt email to the customer and the owner\n"
    "  • Send a low-stock alert if any item is at or below 10\n"
    "  • Clear the cart for this session_id\n"
    "- After `place_order` succeeds, tell the user their order is confirmed and the receipt was emailed.\n"
    "- IMPORTANT: Once `place_order` has been called, consider that purchase CLOSED. "
    "Do NOT attempt to charge again for those same items.\n\n"

    "6) New Orders After a Completed Payment:\n"
    "- If the user wants to buy more items after a completed order, treat it as a NEW order.\n"
    "- First, call `clear_cart` to start fresh.\n"
    "- Add ONLY the new items they request.\n"
    "- Repeat the normal checkout flow for those new items: view cart → generate_summary → trigger_payment_tool "
    "→ wait for payment → stripe_checkout_status_tool → place_order.\n"
